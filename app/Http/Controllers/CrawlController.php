...
class CrawlController extends Controller
{
    ...
    public function onCrawl()
    {
        ...
        // $total = intval(Cache::get($key, 1));
        // if ($total >= 100) {
        //     return to_route('home')->with(['message' => 'exceed the number of crawls allowed for 1 day']);
        // }

        // increment crawl
        // $exp = Carbon::now()->endOfDay()->diffInMinutes(Carbon::now());
        // Cache::put($key, $total + 1, $exp);
        ...
        foreach ($inputs as $input) {
            try {
                $data[] = $this->callAPI($input);
            } catch (\Throwable $th) {
                return to_route('home')->with(['message' => $th->getMessage()]);
            }
        }

        $export = new CrawlExport($data);
        return Excel::download($export, "crawl-data-" . date('ymdhis') . ".xlsx");
    }
}
